...
void strin1(char *p,int n,GLint x,GLint y)
	{
glRasterPos2f(x,y);
for(int i=0;i<n;i++)
...
int p=15,q=265;
glColor3f(2,.6,.6);
strin("Constraints On Input:",21,p,q-10);
strin("_____________________",21,p,q-4-10);
strin("1.Number of processes limited to 5.",35,p,q-(30*1)-10);
strin("2.All processes must arrive before 7sec.",40,p,q-(30*2)-10);
strin("3.Two processes cannot have same Arrival Time.",46,p,q-(30*3)-10);
strin("4.Burst Time must be less than or equal to 5.",45,p,q-(30*4)-10);
glColor3f(.5,1.0,0.0);
strin("PRESS E or e TO RUN PROGRAM FOR STATIC INPUT",44,15,320-10);
    strin("PRESS R or r TO RESET THE PROGRAM",33,15,300-10);
glColor3f(0.0,0.87,.803);
...
for(int u=1;u<=700;u++)
{
	...
	strin("ENTER THE NUMBER OF PROCESSES:",30,15,375-10-10);
	 constr();
	 glColor3f(0.937255,0.870588,0.1099804);
	strin1("Done by Abhijeeth/Abhishek ",27,u,50);
	glFlush();
...
void backgrat()
{
strin("ENTER THE ARRIVAL TIME OF PROCESSES:",35,15,365);
constr();
glFlush();
}//background for getting arrival time
void backgrbt()
{
strin("ENTER THE BRUST TIME OF PROCESSES:",33,15,365);
constr();
glFlush();
